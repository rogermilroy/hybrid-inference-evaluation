...
class KalmanGNN(nn.Module):
    ...
    def __init__(self, h_dim, x_dim, y_dim):
        ...
        self.past_curr_nn = GraphEncoderMLP(2 * self.h_dim, self.h_dim, self.h_dim)
        self.future_curr_nn = GraphEncoderMLP(2 * self.h_dim, self.h_dim, self.h_dim)
        self.y_curr_nn = GraphEncoderMLP(2 * self.h_dim, self.h_dim, self.h_dim)
        ...
        self.decoder = GraphDecoder(self.h_dim, self.h_dim, self.y_dim)
    ...
    def forward(self, hx, graphical_messages):
        ...
        hx_past = torch.cat([hx[0], hx[:-1]]).t()
        hx_future = torch.cat([hx[1:], hx[-1]]).t()
        hx = hx.t()

        # pass compute edge encodings.
        past_curr_edge = self.past_curr_nn(torch.cat([hx_past, hx, past_curr_mess]))
        fut_curr_edge = self.future_curr_nn(torch.cat([hx_future, hx, fut_curr_mess]))
        y_curr_edge = self.y_curr_nn(torch.cat([self.hy, hx, y_curr_mess]))

        # sum edge encodings?
        # pass through node encoder
        h = self.node_nn(sum([past_curr_edge, fut_curr_edge, y_curr_edge]))
    ...
    def initialise_hx_y(self, ys):
        ...
        y_past = torch.cat([ys[0], ys[:-1]]).t()
        y_future = torch.cat([ys[1:], ys[-1]]).t()
        ys = ys.t()
        diff1 = ys - y_past
        diff2 = y_future - ys

        # concatenate and pass through a one dimensional convolution. unsqueeze
        hy_in = torch.cat([diff1, diff2]).t().unsqueeze(0)

        # save as hy
        self.hy = self.hy_initialiser(hy_in.unsqueeze(0)).squeeze(0)  # needs squeezing as convs only take 3d inputs
